...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const analyzeAudio = (): number => {
    ...
    // Create a properly typed Uint8Array for the analyser
    const typedData = new Uint8Array(dataArrayRef.current.buffer);
    // @ts-ignore - TypeScript strictness issue with Uint8Array types
    analyserRef.current.getByteFrequencyData(typedData);

    // Calculate RMS (Root Mean Square) for better voice detection
    let sum = 0;
    for (let i = 0; i < typedData.length; i++) {
      const value = typedData[i] / 255;
      sum += value * value;
    }
    const rms = Math.sqrt(sum / typedData.length);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // @ts-ignore - OGL types are incompatible with standard WebGL types
      ...
      program = new Program(glContext, {
        ...
        uniforms: {
          ...
          iResolution: {
            value: new Vec3(
              glContext.canvas.width || 300,
              glContext.canvas.height || 300,
              (glContext.canvas.width || 300) / (glContext.canvas.height || 300)
            ),
          },
          hue: { value: hue },
          hover: { value: 0 },
